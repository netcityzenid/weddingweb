...
const SwipeCard: React.FC = () => {
  ...
  const [selectedImage, setSelectedImage] = useState<string | null>(null); // State untuk menyimpan URL gambar yang di-klik
  ...
  const handleImageClick = (url: string) => {
    setSelectedImage(url); // Set gambar yang di-klik
  };

  // Handle closing modal
  const handleClosePreview = () => {
    setSelectedImage(null); // Reset gambar ketika modal ditutup
  };

  return (
    <div className="flex justify-center pt-5 h-[700px] bg-red-500 relative">
      {/* Render reload button behind the cards */}
      {cards.slice(0, currentIndex + 1).map((card, index) => (
        <TinderCard key={card.id} onSwipe={(dir) => swiped(dir, card.id)} onCardLeftScreen={() => outOfFrame(card.id)} preventSwipe={index === cards.length - 1 ? [] : ["up", "down"]}>
          <div
            className="border-2 border-[#000] absolute cursor-pointer"
            style={{
              width: "300px",
              height: "400px",
      ...
      {selectedImage && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-75 z-50 px-5">
          <div className="relative">
            <img src={selectedImage} alt="Preview" className="max-w-full max-h-screen rounded-md " />
            <button className="absolute top-2 right-2 text-white " onClick={handleClosePreview}>
              <svg className="h-10 opacity-60 w-auto" width="64px" height="64px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
